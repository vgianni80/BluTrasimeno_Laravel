...
<head>
    ...
    <title>Prenota - {{ $propertyName }}</title>
...
<body>
                        ...
                        <form method="POST" action="{{ route('public.booking.store') }}" id="bookingForm">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="guests" id="guests" class="form-select" required>
                                    @for($i = 1; $i <= ($maxGuests ?? 6); $i++)
                                        <option value="{{ $i }}" {{ $i == 2 ? 'selected' : '' }}>{{ $i }} {{ $i == 1 ? 'ospite' : 'ospiti' }}</option>
                                    @endfor
                                </select>
                            </div>
    ...
    <script>
        ...
        async function loadCalendar() {
            ...
            try {
                const response = await fetch(`{{ route('public.booking.calendar') }}?year=${currentYear}&month=${currentMonth}`);
